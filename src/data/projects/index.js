...
// Group by semester folder (assumes files are in ./<semester-folder>/<project>.json)
const groups = {};
...
for (const [path, mod] of Object.entries(modules)) {
  ...
  // path looks like './2024-sem1/winter-hackathon.json' or './Sem 2, 2026/project.json'
...
let semesters = Object.keys(groups).map((folder) => {
  ...
  const projects = groups[folder].slice().sort((a, b) => new Date(b.date) - new Date(a.date));
  return {
    id: folder,
    folder,
    label: parsed.label,
